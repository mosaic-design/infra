...
export const runCliCommands = () => {
    ...
    program
        ...
        .option('-o, --repo-owner <string>', 'github owner name', process.env['MOSAIC_REPO_OWNER'] ?? 'mosaic-design')
        .option('-t, --repo-token <string>', 'github repo token', process.env['MOSAIC_REPO_TOKEN'] ?? process.env['GITHUB_TOKEN'] )
        .option('-n, --repo-name <string>', 'github repo name', process.env['MOSAIC_REPO_NAME'] ?? 'infraasfasf')
